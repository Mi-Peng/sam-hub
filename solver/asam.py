...
class ASAM(Optimizer):
    @configurable
    def __init__(self, params, base_optimizer, rho, **kwargs):
    ...
    def perturb_weights(self):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                e_w = torch.pow(p.clone().detach(), 2) * p.grad * scale
    ...
    def _grad_norm(self):
        ...
        norm = torch.norm(
                    torch.stack([
                        (torch.abs(p.clone().detach()) * p.grad).norm(p=2).to(shared_device)
